...
export default function ParcoursPage() {
  ...
  const selectedCap = useMemo(
    () => parcoursCaps.find((cap) => cap.id === selectedId) ?? current ?? parcoursCaps[0],
    [current, selectedId],
  );
  const getDropRect = useCallback(
    () => dropRef.current?.getBoundingClientRect() ?? null,
    [],
  );
  ...
  return (
    <section className="section parcours-page">
      <h1>Parcours</h1>
      <p className="muted">
        Glissez une bulle dans l’écran ou cliquez pour afficher le détail de l’étape.
      </p>
      ...
      <div className="parcours-layout">
        ...
        {selectedCap && (
          <DropScreen ref={dropRef} cap={selectedCap} isDropActive={isDropActive} />
        )}
      </div>
    </section>
  );
